...
_MAX_SAMPLES = 200
_latencies: dict[str, deque] = defaultdict(lambda: deque(maxlen=_MAX_SAMPLES)) # type: ignore

# ── Error tracking (rolling window) ───────────────────────────────────────────
_MAX_ERRORS = 100
_recent_errors: deque = deque(maxlen=_MAX_ERRORS) # type: ignore
...
def get_metrics() -> dict[str, Any]:
    ...
    with _lock:
        ...
        for key, samples in _latencies.items(): # type: ignore
            if samples:
                sorted_s = sorted(samples) # type: ignore
                n = len(sorted_s) # type: ignore
                latency_summaries[key] = {
                    "count":  n,
                    "avg_ms": round(sum(sorted_s) / n, 1), # type: ignore
                    "p50_ms": round(sorted_s[n // 2], 1), # type: ignore
                    "p95_ms": round(sorted_s[int(n * 0.95)], 1), # type: ignore
                    "p99_ms": round(sorted_s[int(n * 0.99)], 1), # type: ignore
                    "max_ms": round(sorted_s[-1], 1), # type: ignore
                }
        return {
            ...
            "recent_errors": list(_recent_errors)[-20:], # type: ignore
        }
