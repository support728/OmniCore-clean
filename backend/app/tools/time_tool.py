...
def handle_time(message: str) -> str:
    lower = message.lower()
    tz_map = {
    ...
    for city, (label, offset) in tz_map.items():
        if city in lower:
            now = datetime.now(timezone(timedelta(hours=offset)))
            return f"The current time in {city.title()} is approximately {now.strftime('%I:%M %p, %A %B %d, %Y')} ({label})."
