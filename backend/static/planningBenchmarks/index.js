...
function createBenchmarkCollector(label) {
  ...
  return {
    ...
    recordWorkflowCompleted: function () {
    ...
    finish: function (operations) {
      ...
      var totalLatency = latencies.reduce(function (sum, current) { return sum + current; }, 0);
      return {
        label: label,
...
function formatBenchmarkReport(summaries) {
  ...
  for (var i = 0; i < summaries.length; i++) {
    ...
    lines.push("  completed workflows: " + summary.completedWorkflows);
